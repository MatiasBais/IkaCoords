...
<td>
    <select name="fecha" id="fecha">
        <?php
        $sql = "SELECT numero, DATE_FORMAT(fecha,'%d/%m/%y') as 'Fecha' FROM updates order by numero desc";
...
<table class="results">
        ...
        $order="";
		if($cla=="Totales")
			$order="sum(((b.Constructor+b.Investigadores+b.Generales)-(a.Constructor+a.Investigadores+a.Generales)))";
		if($cla=="Constructor")
			$order="sum((b.Constructor-a.Constructor))";
		if($cla=="Investigadores")
			$order="sum((b.Investigadores-a.Investigadores))";
		if($cla=="Generales")
			$order="sum((b.Generales-a.Generales))";
		if($cla=="Oro")
			$order="sum((b.Oro-a.Oro))";
        
    

        $sql = "select alianza.nombre as 'ali', DATE_FORMAT(ub.fecha,'%d/%m/%y') as 'Fecha',
        FORMAT(sum((b.Constructor+b.Investigadores+b.Generales)),0)as 'Totales',
        FORMAT(sum(((b.Constructor+b.Investigadores+b.Generales)-(a.Constructor+a.Investigadores+a.Generales))),0) as 'Δ-Totales' ,
        FORMAT(sum(b.Constructor),0) as 'Constructor', 
        FORMAT(sum((b.Constructor-a.Constructor)),0) as 'Δ-Constructor',
        FORMAT(sum(b.investigadores),0) as 'Investigadores', 
        FORMAT(sum((b.Investigadores-a.Investigadores)),0) as 'Δ-Investigadores', 
        FORMAT(sum(b.Generales),0) as 'Generales', 
        FORMAT(sum((b.Generales-a.Generales)),0) as 'Δ-Generales', 
        FORMAT(sum(b.oro),0) as 'Oro', 
        FORMAT(sum((b.Oro-a.Oro)),0) as 'Δ-Oro' ,ua.numero,ub.numero 
        from player 
        ...
        join alianza on player.idAlianza = alianza.idalianza 
        where ub.numero = (select max(numero) from updates where server = '".$server."') 
        and ua.numero= (select min(numero) from updates where numero>=".$fecha." and  server = '".$server."') and player.server='".$server."' and ua.server='".$server."' and ub.server='".$server."' and alianza.server='".$server."'
         group by alianza.idalianza
                order by ".$order." desc 
                limit ".$offset.",50";
        $result = $conn->prepare($sql);
        $result->execute();
